...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // إضافة خبر مع دعم روابط المرفقات الثلاثة المنفصلة
    ...
    if(addNewsForm) {
        addNewsForm.addEventListener('submit', async (e) => {
            ...
            const videoUrl = document.getElementById('news-video').value;       // الرابط الجديد للفيديو
            const documentUrl = document.getElementById('news-document').value; // الرابط الجديد للمستند
            const category = document.getElementById('news-category').value;
            const status = document.getElementById('news-status').value;

            try {
                // إرسال كائن الخبر شاملاً الحقول الثلاثة المستقلة
                ...
                alert('تم نشر الخبر بنجاح شامل الأقسام والمرفقات الجديدة!');
                addNewsForm.reset();
                loadAdminNews(); 
            } catch (error) {
